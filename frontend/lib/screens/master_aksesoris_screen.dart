...
class MasterAksesorisScreen extends StatefulWidget {
  final String? role;
  const MasterAksesorisScreen({super.key, this.role});

  @override
  State<MasterAksesorisScreen> createState() => _MasterAksesorisScreenState();
}

class _MasterAksesorisScreenState extends State<MasterAksesorisScreen> {
        ...
        body: SafeArea(
          child: Column(
            ...
            children: [
              ...
              Expanded(
                child: StreamBuilder<List<AksesorisItemModel>>(
  ...
  Widget _buildAksesorisCatalogList(List<AksesorisItemModel> filteredItems) {
...
class _AksesorisFormDialogState extends State<AksesorisFormDialog> {
  ...
  bool _isEdited = false;
  final FirebaseService _firebaseService = FirebaseService();
  List<AksesorisItemModel> _existingItemsList = [];
  ...
  void initState() {
    ...
    _firebaseService.accessoriesStream.first.then((items) {
  ...
  void _checkDuplicate() {
    ...
    bool found = _existingItemsList.any((item) => 
      item.name.toLowerCase() == name && item.price == price && item.id != widget.existingItem?.id
    );
